...
from garage.causallearnmain.causallearn.search.FCMBased import lingam
import pandas as pd
import time
# yapf: enable


class Causal_CAE(RLAlgorithm):
        ...
        for _ in trainer.step_epochs():
            for _ in range(self._steps_per_epoch):
                if not (self.replay_buffer.n_transitions_stored >=
                    ...
                    batch_size = None
                #     here: use the step_episode for causal model learning

                trainer.step_episode = trainer.obtain_samples(
                    trainer.step_itr, 5000)
                causal_weight = np.ones(4, dtype=np.float32)
                print('len(trainer.step_episode)', len(trainer.step_episode))
                if len(trainer.step_episode) >= 5000:
                    causal_weight, causal_computing_time = self.get_sa2r_weight(trainer.step_episode, 5000, 'DirectLiNGAM')
                    print('causal_computing_time', causal_computing_time)
                ...
                for _ in range(self._gradient_steps):
                    policy_loss, qf1_loss, qf2_loss = self.train_once(causal_weight)
    ...
    def extract_from_samples(self, samples, sample_size):
        states = []
        actions = []
        rewards = []
        next_states = []
        ...
        for path in samples:
            ...
            rewards.append(path['rewards'].reshape(-1, 1))
            next_states.append(path['next_observations'])
            dones.append(np.array(
    ...
    def get_sa2r_weight(self, memory, sample_size=5000,
                        causal_method='DirectLiNGAM'):
        states, actions, rewards, next_states, dones = self.extract_from_samples(memory,
            sample_size)
        print('states', states)
        ...
        X_ori = np.hstack(
            (states[:sample_size, :], actions[:sample_size, :], rewards))
        ...
        if causal_method == 'DirectLiNGAM':
            ...
            weight_r = model.adjacency_matrix_[-1, np.shape(states)[1]:(
                    np.shape(states)[1] + np.shape(actions)[1])]
    ...
    def train_once(self, causal_weight=None, itr=None, paths=None):
        ...
        if self.replay_buffer.n_transitions_stored >= self._min_buffer_size:
            ...
            policy_loss, qf1_loss, qf2_loss = self.optimize_policy(samples, causal_weight)
    ...
    def _critic_objective(self, samples_data, causal_weight):
        ...
        # y_t: new_next_actions_pre_tanh action: new_next_actions
        new_next_actions_pre_tanh, new_next_actions = (new_next_actions_dist.rsample_with_pre_tanh_value())
        new_log_pi = new_next_actions_dist.log_prob_causal(
            value=new_next_actions, pre_tanh_value=new_next_actions_pre_tanh, causal_weight=causal_weight)

        # new_log_pi -> causal new_log_pi
    ...
    def optimize_policy(self, samples_data, causal_weight):
        ...
        """
        obs = samples_data['observation']
        qf1_loss, qf2_loss = self._critic_objective(samples_data, causal_weight)
        ...
        log_pi_new_actions = action_dists.log_prob_causal(
            value=new_actions, pre_tanh_value=new_actions_pre_tanh, causal_weight=causal_weight)
